...
{
    public class FromErpAuthentication : WinwinApiBase
    {
        private readonly RequestDelegate _next;
        //[s]這邊建議token放在config在透過DI方式注入進來使用 請參考pudo專案注入方式
        private readonly string ToolToken = "u2wW5v2dbrH98dApzZyDFS5xQuQvgvTP";
        public FromErpAuthentication(RequestDelegate next)
        {
            _next = next;
        }
        ...
        private async Task<bool> TokenAuthentication(HttpContext context)
        {
            ...
            try
            {
                string token = context.Request.Headers["erp-token"];
